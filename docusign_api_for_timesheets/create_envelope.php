...
########## Temp ##########--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// if(isset($_POST['create_envelope'])){
//     $envelope = new CreateEnvelope;
//     $envelop->public_create_envelope($document_name, $employee_name);
// }
...
class CreateEnvelope{
    ...
    public function public_create_envelope($document_name, $employee_name){ #BXP: Public access to private method
        $this->create_envelope($document_name, $employee_name);
    }

    private function create_envelope($document_name, $employee_name){ #BXP: Private access (class members not accessible outside the scope)
        ...
        $document_path = join("/", array($_SERVER['DOCUMENT_ROOT'], "files", "time_keeping", $document_name . ".pdf")); #BXP: Replace "\\" separator with "/" for live version
        ...
        $signer_email = "***"; #BXP: $_SESSION["signer_email"];
        ...
        $document = new DocuSign\eSign\Model\Document([  
            ...
            'name' => $document_name . '.pdf', #BXP: Alternatively ask user to enter a name - Eliminate file extension if needed
            'file_extension' => 'pdf',
        ...
        $sign_here = new DocuSign\eSign\Model\SignHere([ 
            ...
            'anchor_string' => 'Signature:', 'anchor_units' => 'pixels', 'anchor_y_offset' => '0', 'anchor_x_offset' => '75' #BXP: Anchor text positioning - Replace 'Signature:' with anything
        ]);
        ...
        $envelope_definition = new DocuSign\eSign\Model\EnvelopeDefinition([
            'email_subject' => "Please DocuSign Your Timesheet",
            'email_blurb' => "Please DocuSign your timesheet. Thank you!",
        ...
        // $document_name = $_SESSION['time_sheet_name'];
        // $sql = "INSERT INTO signers (signer_name, document_name, envelope_id) VALUES ('$signer_name', '$document_name', '$envelope_id')";
        // $connection->query($sql); #BXP: Alternativly use mysqli_query($connection, $sql);
        ...
        ########## Next Steps ##########--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        header('location: /time_keeping_test.php?notification=mtsss'); #BXP: add a success message for DS
    }
}
?>